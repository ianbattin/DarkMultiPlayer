...
namespace DarkMultiPlayerCommon
{
	public class Common {
		...
		public static string serializedContract(Contract contract) {
		...
		public static Contract deserializedContract(string data) {
			byte[] b = Convert.FromBase64String(data);
			using (MemoryStream stream = new MemoryStream(b)) {
				BinaryFormatter formatter = new BinaryFormatter();
				stream.Seek(0, SeekOrigin.Begin);
				return (Contract)formatter.Deserialize(stream);
			}
		}

		public static List<string> serializeArrayContracts(List<Contract> contracts) {
			List<string> serializedContracts = new List<string>();
			foreach (Contract contract in contracts) {
				using (MemoryStream stream = new MemoryStream()) {
					...
					stream.Flush();
					stream.Position = 0;
					serializedContracts.Add(Convert.ToBase64String(stream.ToArray()));
				}
			}
		...
		public static List<Contract> deserializeArrayContracts(List<string> contractData) {
			List<Contract> deserializedContracts = new List<Contract>();
			foreach (string data in contractData) {
				byte[] b = Convert.FromBase64String(data);
				using (MemoryStream stream = new MemoryStream(b)) {
					BinaryFormatter formatter = new BinaryFormatter();
					stream.Seek(0, SeekOrigin.Begin);
					deserializedContracts.Add((Contract)formatter.Deserialize(stream));
				}
			}

			return deserializedContracts;
		}
	}
    ...
    public class TeamStatus
    {
		...
		public List<List<Contract>> contracts = new List<List<Contract>>();
		private List<Contract> accepted = new List<Contract>();
		private List<Contract> cancelled = new List<Contract>();
		private List<Contract> completed = new List<Contract>();
		private List<Contract> declined = new List<Contract>();
		private List<Contract> failed = new List<Contract>();
		private List<Contract> finished = new List<Contract>();
		private List<Contract> offered = new List<Contract>();
